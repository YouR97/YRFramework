...
namespace SuperScrollView
{
    ...
    public class LoopGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
       ...
       //fetch or create a new item form the item pool.
        public LoopGridViewItem NewListViewItem(string itemPrefabName, int index)
        {
            if (!mItemPoolDict.TryGetValue(itemPrefabName, out GridItemPool pool))
            {
                Debug.LogError($"{nameof(LoopGridView)}未获取到名字为{itemPrefabName}的模板");
                return null;
            }
